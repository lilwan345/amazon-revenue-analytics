...
*The risk ranking comes from a simple model (logistic regression) that scores each household's chance of going quiet next quarter, using only its behavior through mid-2022, and is then checked against what actually happened in Q3 (AUC 0.90). The inputs never see Q3, so the ranking isn't hindsight. (How the model is kept from peeking at the future, and why that 0.90 is an in-sample number, is in [METHODOLOGY.md](METHODOLOGY.md).)*
