...
A hypothetical Amazon Retail Finance team needs three things to plan next quarter's budget:
...
Each question gets its own analytical layer. My role is the decision-support inputs — not the forecast, the budget decision, or the P&L.

## The Answer

**Layer 1 — concentration sits at the top.** In this 5,026-household panel, the top 10% of households drive **35.9% of GMV** (gross merchandise value — total household spend; top 20%: 55%; Gini 0.528, a standard 0–1 concentration score). That is real concentration, but short of a classic 80/20 split — the long tail still matters, and a VIP-only view would miss nearly two-thirds of GMV. The gap is almost all *how often people buy*, not how much per order: top-decile households buy ~11× more often but spend only ~1.1× more per purchase. And the clearest marker of a top-decile household is how often they shop, not how much they earn — frequent shoppers (>10×/month) stand out far more than high earners ($150K+) do. Concentration actually *fell* slightly during COVID while panel GMV nearly doubled (2018 $5.5M → 2022 $10.7M): the surge was mass-market, not VIP-only.
...
These 5,026 households are essentially the **full consenting panel** — not a random sample of Amazon's customers — so every number reads as "within this panel," never "across Amazon." The panel is more engaged than Amazon's base (87% Q3 activity), so engagement figures are an upper bound. "Revenue-at-risk" is a single-quarter *expected* figure — each household's chance of going quiet × what it would have spent — not a worst-case number, and not permanent lost revenue. Demographics are a one-time 2021 snapshot.
...
The 11 super-categories (plus an explicit `Other / Unknown` bucket) are rolled up from 1,871 raw Amazon category labels via a committed Claude Opus 4.7 taxonomy — coverage and false-positive audit are in [METHODOLOGY.md](METHODOLOGY.md).
...
- **Pet & Auto** grow from existing customers buying deeper, not new ones arriving — a retention story, not acquisition.
...
- **Consenting panel, not Amazon's customer base.** Every number is "within this 5,026-household panel," never "across Amazon." People who consent to share purchase data likely differ from those who don't — the panel's 87% Q3 activity rate is direct evidence it is more engaged than Amazon's base.
- **Panel attrition vs. purchase attrition are not separable.** A household at zero GMV in a later quarter could have stopped buying on Amazon *or* stopped reporting to the survey vendor — both look identical here. The 2023-01-01 cohort cap limits the worst of it but cannot fully separate the two.
- **Single-quarter outcome, not churn.** Layer 2 measures "no purchase in 2022-Q3," not permanent churn — ~28% of households silent for a year reactivate within a quarter. The wording stays "Q3 drop-off" throughout, never "churn."
- **Cohort cap at 2023-01-01.** Post-2023 data is sparse (~2% of rows) due to participants leaving the panel; including it would right-censor people who simply stopped reporting.
