...
- **What the 0.90 is and isn't.** The model is fit on the Q3 2022 outcome and scored on that same quarter, so the AUC is in-sample. The optimism from that is small: fitting the same model to randomly shuffled labels only reaches AUC ~0.53 (chance is 0.50). And a single feature, days since last purchase, already reaches 0.86, so most of the accuracy reflects how predictable one-quarter drop-off is. The strict out-of-time test (fit on Q2, score Q3) is a natural next step I haven't run.
