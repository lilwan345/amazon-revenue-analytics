...
- **Two engines, cross-checked.** Each core total is computed in SQL (DuckDB, in `sql/`) and again in Polars (in the notebook), and the two are compared before anything is saved. If they disagree, something is wrong — it's a built-in sanity check.
...
- **Frequency vs. basket.** The top decile's GMV lead splits into "how often they buy" and "how much per order." Here it is ~95% the first and ~5% the second — the top decile is defined by buying *more often*, not by bigger baskets.
...
- **"Q3 drop-off," not "churn."** The outcome is simply "no purchase in 2022-Q3." It is *not* permanent churn — a quick check found ~28% of households silent for a full year buy again within a quarter. The wording stays "drop-off" throughout to keep that honest.
