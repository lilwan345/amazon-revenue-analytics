...
> Committed audit document. The notebooks don't rewrite it; notebook 01 re-checks the
> input hashes below on every run (`src/manifest_utils.py::verify_inputs`).
...
matching Finder and the Dataverse source — 313.07 MB = 298.57 MiB). Hashes are plain SHA256
(`shasum -a 256 data/raw/*.csv` prints the same digests) -- see `src/manifest_utils.py::file_sha256`.
...
| `category_taxonomy_mapping.csv` | 1,816 | 2 | raw_category, super_category (flattened JSON for SQL JOIN) | flattened from taxonomy JSON |
...
| `sql/07_category_rollup.sql` | Layer 3: super-category × year GMV rollup over the cohort-capped panel (`purchases` view) |
| `src/data_loader.py` | Polars / DuckDB loaders; registers the `purchases` / `survey` views the SQL reads |
| `src/manifest_utils.py` | SHA256 hashing; read-only check of the raw inputs against this file |
...
| `src/robustness_checks.py` | Layer 2 held-out AUC (5-fold CV, Q2→Q3 walk-forward backtest) + bootstrap CIs on Gini / top-decile share → `outputs/robustness_checks.json` |
...
- Layer 2 segment-RaR CIs: 1,000 iter × 52 segments = 52K resamples, ~0.3 sec.

## Reproducibility Notes

- **Random seeds:** All stochastic operations seed=42 (bootstrap, shuffles, model training). Shuffle-label diagnostic uses seed=123 to differentiate from main bootstrap. Every bootstrap indexes rows in a fixed order (sorted household IDs), so CIs are identical run to run.
...
| 2026-09-24 | all | Layer 3 bootstrap CIs made reproducible (households were resampled in hash/set order, so CIs drifted between runs); `python -m src.build_layer3` import fixed; notebook 03 no longer hard-codes a local path; sql/07 reads the shared `purchases` view; notebooks stop rewriting this file and verify input hashes instead; stale truncated-copy numbers removed from notebook narrative (incl. a leftover 2,845-household divisor in notebook 02); raw label count corrected to 1,816 (the committed mapping) |
| 2026-09-24 | 2 | Held-out AUC documented (5-fold CV 0.902 ± 0.011, 5×10 repeated 0.902 [folds 0.868–0.926], 80/20 holdout 0.887 on 1,006 households); `src/robustness_checks.py` recomputes them plus a Q2→Q3 walk-forward backtest and bootstrap CIs on Gini / top-decile share |
